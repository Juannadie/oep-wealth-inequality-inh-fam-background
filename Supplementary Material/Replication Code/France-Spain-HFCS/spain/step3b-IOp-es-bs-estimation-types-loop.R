...
temp = list.files(pattern="v2-HFCS14_Spain_after_step_2d-no-pension-ad-eq_*") #creates a list with all the personal files in the eu-silc directory directory
...
write.csv(results4typesbsESfull, file = "results/spain/es-results4types-full-imp1.csv")
write.table(results4typesbsESfull, file = "results/spain/es-results4types-full-imps.csv", append = T, sep = ",", row.names = T)

results6typesbsESfull <- rbind(indextheil6t, indextheil6tIE, indexshare6t)

write.csv(results6typesbsESfull, file = "results/spain/es-results6types-full-imp1.csv")
write.table(results6typesbsESfull, file = "results/spain/es-results6types-full-imps.csv", append = T, sep = ",", row.names = T)


results24typesbsESfull <- rbind(indextheil24t, indextheil24tIE, indexshare24t)

write.csv(results24typesbsESfull, file = "results/spain/es-results24types-full-imp1.csv")
write.table(results24typesbsESfull, file = "results/spain/es-results24types-full-imps.csv", append = T, sep = ",", row.names = T)

resultsmarginalES <- rbind(indexmarginal_bk, indexmarginal_inh )

write.csv(resultsmarginalES, file = "results/spain/es-results-marginal-full-imp1.csv")
write.table(resultsmarginalES, file = "results/spain/es-results-marginal-full-imps.csv", append = T, sep = ",", row.names = T)


resultsinteractiveES <- indexinteractive_bk_inh

write.csv(resultsinteractiveES, file = "results/spain/es-results-interactive-full-imp1.csv")
write.table(resultsinteractiveES, file = "results/spain/es-results-interactive-full-imps.csv", append = T, sep = ",", row.names = T)


resultsshapleyES <- rbind(indexshapley_bk, indexshapley_inh )

write.csv(resultsshapleyES, file = "results/spain/es-results-shapley-full-imp1.csv")
write.table(resultsshapleyES, file = "results/spain/es-results-shapley-full-imps.csv", append = T, sep = ",", row.names = T)
...
ggsave(file="graphs/v2-4types-ES-no-pension-ad-eq-imp1.pdf", device = "pdf", scale = 1, width = 7.5, height = 5, units = ("in"), dpi = 400, limitsize = TRUE)
