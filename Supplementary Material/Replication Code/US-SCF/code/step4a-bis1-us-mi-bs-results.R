...
sesimps <- read.csv(file = "results/us/bis1/us-results4types-full-imps.csv", row.names = NULL)
...
inhimps <- read.csv(file = "results/us/bis1/us-results6types-full-imps.csv", row.names = NULL)
...
jointimps <- read.csv(file = "results/us/bis1/us-results24types-full-imps.csv", row.names = NULL)
...
us_res <- rbind (w_ineq_res, ses_ineq_res, ses_share_res, inh_ineq_res, inh_share_res, joint_ineq_res, joint_share_res)

colnames(us_res) <- c("estimate", "se", "low_ci", "high_ci")

write.csv (us_res, file = "results/us/bis1/us_res.csv")