...
sesimps <- read.csv(file = "results/uk_bis1/uk-results4types-full-imps.csv", row.names = NULL)
...
inhimps <- read.csv(file = "results/uk_bis1/uk-results6types-full-imps.csv", row.names = NULL)
...
jointimps <- read.csv(file = "results/uk_bis1/uk-results24types-full-imps.csv", row.names = NULL)
...
uk_res <- rbind (w_ineq_res, ses_ineq_res, ses_share_res, inh_ineq_res, inh_share_res, joint_ineq_res, joint_share_res)

colnames(uk_res) <- c("estimate", "se", "low_ci", "high_ci")

write.csv (uk_res, file = "results/uk_bis1/uk_res.csv")
