...
temp = list.files(pattern="WAS-After-Step-v2-IO-8b-imp*") #creates a list with all the personal files in the eu-silc directory directory
...
write.csv(results4typesbsUKfull, file = "results/uk/uk-results4types-full-imp1.csv")
write.table(results4typesbsUKfull, file = "results/uk/uk-results4types-full-imps.csv", append = T, sep = ",", row.names = T)

results6typesbsUKfull <- rbind(indextheil6t, indextheil6tIE, indexshare6t)

write.csv(results6typesbsUKfull, file = "results/uk/uk-results6types-full-imp1.csv")
write.table(results6typesbsUKfull, file = "results/uk/uk-results6types-full-imps.csv", append = T, sep = ",", row.names = T)



results24typesbsUKfull <- rbind(indextheil24t, indextheil24tIE, indexshare24t)

write.csv(results24typesbsUKfull, file = "results/uk/uk-results24types-full-imp1.csv")
write.table(results24typesbsUKfull, file = "results/uk/uk-results24types-full-imps.csv", append = T, sep = ",", row.names = T)


resultsmarginalUK <- rbind(indexmarginal_bk, indexmarginal_inh )

write.csv(resultsmarginalUK, file = "results/uk/uk-results-marginal-full-imp1.csv")
write.table(resultsmarginalUK, file = "results/uk/uk-results-marginal-full-imps.csv", append = T, sep = ",", row.names = T)


resultsinteractiveUK <- indexinteractive_bk_inh

write.csv(resultsinteractiveUK, file = "results/uk/uk-results-interactive-full-imp1.csv")
write.table(resultsinteractiveUK, file = "results/uk/uk-results-interactive-full-imps.csv", append = T, sep = ",", row.names = T)


resultsshapleyUK <- rbind(indexshapley_bk, indexshapley_inh )

write.csv(resultsshapleyUK, file = "results/uk/uk-results-shapley-full-imp1.csv")
write.table(resultsshapleyUK, file = "results/uk/uk-results-shapley-full-imps.csv", append = T, sep = ",", row.names = T)
...
ggsave(file="graphs/uk-4types-UK-no-pension-ad-eq-imp1.pdf", device = "pdf", scale = 1, width = 7.5, height = 5, units = ("in"), dpi = 400, limitsize = TRUE)
