...
setwd("/Users/Juan/Google Drive/A-UK-Research/IO-Wealth-All-Countries/WAS-IOp/code") #Set Working Directory


shapleyimps <- read.csv(file = "results/uk_bis2/uk-results-shapley-full-imps.csv", row.names = NULL)
...
marginalimps <- read.csv(file = "results/uk_bis2/uk-results-marginal-full-imps.csv", row.names = NULL)
...
interactiveimps <- read.csv(file = "results/uk_bis2/uk-results-interactive-full-imps.csv", row.names = NULL)
...
uk_res_2 <- rbind (marginal_ses_res, marginal_inh_res, shapley_ses_res, shapley_inh_res, interactive_res)

colnames(uk_res_2) <- c("estimate", "se", "low_ci", "high_ci")

write.csv (uk_res_2, file = "results/uk_bis2/uk_res_2.csv")
